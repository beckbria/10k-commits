...
class Commit:
    ...
    def __str__(self):
        return "%s\t%s\t%s\t%s\t%s\t%s" % (
            self.hash,
            str(self.order),
            self.original_parent_hash,
            str(self.original_parent_order),
            self.author_time,
            self.commit_time,
        )
...
class GitParser:
        ...
        if blob_id not in self.blobs:
            ...
            for b in self.blobs:
                if b.startswith(blob_id):
                    if full_id != blob_id:
                        print(
                            "ERROR: Got ambiguous short blob ID ",
                            blob_id,
                            " not found (ref. commit ",
                            hash,
                            " - tree ",
                            commit.tree_id,
                            ") which matches ",
    ...
    def parseObject(self, hash, contents):
        ...
        if match := GitParser._blob_re.search(contentsStr):
            ...
            self.commits[hash] = GitCommit(
                num=match.group(1),
                tree_id=match.group(2),
                parent_id=match.group(4),
                author=match.group(5),
                author_time=match.group(6),
                committer=match.group(7),
                commit_time=match.group(8),
            )
        elif match := GitParser._tree_re.search(contentsStr):
            self.trees[hash] = GitTree(
                num=match.group(1), data=match.group(2), binary_data=contents
            )
        else:
            print("*** UNKNOWN FILE: ", hash, " *** ", contentsStr)
    ...
    def _buildOrderedCommit(self, commit_id):
        ...
        if (
            commit_data.parent_id is not None
            and commit_data.parent_id not in self.commits
        ):
        ...
        return Commit(
            hash=commit_id,
            order=int(blob.commit_order),
            original_parent_hash=commit_data.parent_id,
            original_parent_order=original_parent_order,
            author_time=commit_data.author_time,
